...
function getBlocks(state, district) {
...
function getMandis(state, district, block) {
  if (!state || !district) return [];
...
let districtMap = 'INDIA_DISTRICT_MAP: dict[str, tuple[str, str]] = {\n';
for (const [state, data] of Object.entries(INDIA_DATA)) {
  for (const dist of data.districts) {
    districtMap += `    "${dist.toLowerCase()}": ("${state}", "${dist}"),\n`;
  }
}
districtMap += '}\n';

let stateMap = 'INDIA_STATE_MAP: dict[str, str] = {\n';
for (const state of Object.keys(INDIA_DATA)) {
  stateMap += `    "${state.toLowerCase()}": "${state}",\n`;
}
stateMap += '}\n';

fs.writeFileSync('maps.py', districtMap + '\n' + stateMap);
